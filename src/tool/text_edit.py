...
class TextEditor:
    """基于 ITerminal 的文本行级修改工具类（支持多文件+新建控制）。
    ...
    6. 检查终端的 allow_commands 与自身 allow_commands 的一致性；
    7. 检查终端是否禁用脚本执行（确保安全性）。
    """

    def __init__(self, terminal: ITerminal, allow_commands: Optional[List[str]] = None) -> None:
        """初始化文本编辑器，仅绑定终端实例（不固定文件路径）。
    ...
    def edit(self, file_path: str, operations: List[EditOperation]) -> None:
        ...
        for _, op in processed_ops:
            ...
            if op.op == "delete":
                ...
                cmd = f"sed {''.join(self._sed_inplace_arg)} '{op.line}c\\{escaped_content}\\' {file_rel_quoted}"
            elif op.op == "insert":
                if op.line == 0:
                    # 插入到文件开头：使用临时文件方法避免sed语法问题
                    cmd = f"(echo '{escaped_content}'; cat {file_rel_quoted}) > {file_rel_quoted}.tmp && mv {file_rel_quoted}.tmp {file_rel_quoted}"
                elif op.line == -1:
                    # 插入到文件末尾：echo >> file
                    cmd = f"echo '{escaped_content}' >> {file_rel_quoted}"
                else:
                    # 插入到第 N 行之前：sed -i '{line}i\内容' file
                    cmd = f"sed {''.join(self._sed_inplace_arg)} '{op.line}i\\{escaped_content}\\' {file_rel_quoted}"
            else:
...
if __name__ == "__main__":
    try:
        ...
        # 2. 初始化文本编辑器（仅绑定 Terminal，不固定文件）
        editor = TextEditor(terminal=terminal)
        print(f"✅ 文本编辑器初始化完成（支持多文件编辑）\n")
