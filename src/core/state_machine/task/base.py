...
from ....model import Message, TextBlock, ImageBlock, VideoBlock
...
class BaseTask(BaseStateMachine[StateT, EventT], ITask[StateT, EventT]):
    ...
    def get_protocol(self) -> list[TextBlock | ImageBlock | VideoBlock]:
...
class ProtocolTaskView(ITaskView[StateT, EventT]):
    ...
    def __call__(self, task: ITask[StateT, EventT], **kwargs: Any) -> str:
        ...
        # 将 protocol 列表转换为字符串
        protocol_str = "\n".join(
            block.text if isinstance(block, TextBlock) else f"[{block.__class__.__name__}]"
