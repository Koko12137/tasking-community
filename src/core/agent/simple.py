...
from src.core.state_machine.workflow import SimpleStage, SimpleEvent, IWorkflow, BaseWorkflow
...
def get_simple_transition() -> dict[
    tuple[SimpleStage, SimpleEvent],
    tuple[
        SimpleStage,
        Callable[[IWorkflow[SimpleStage, SimpleEvent, TaskState, TaskEvent]], Awaitable[None] | None] | None
    ]
]:
    ...
    def on_processing_to_processing(
        workflow: IWorkflow[SimpleStage, SimpleEvent, TaskState, TaskEvent],
    ) -> None:
    ...
    # 添加转换规则
    transition[(SimpleStage.PROCESSING, SimpleEvent.PROCESS)] = (
        SimpleStage.PROCESSING, on_processing_to_processing
    )
    ...
    # 添加转换规则
    transition[(SimpleStage.PROCESSING, SimpleEvent.COMPLETE)] = (
        SimpleStage.COMPLETED, on_processing_to_completed
    )

    return transition


def get_simple_stages() -> set[SimpleStage]:
    """获取常用工作流阶段集合
    ...
    """
    return {
        SimpleStage.PROCESSING,
        SimpleStage.COMPLETED,
    }


def get_simple_event_chain() -> list[SimpleEvent]:
    """获取常用工作流事件链
    -  PROCESS, COMPLETE
    ...
    return [
        SimpleEvent.PROCESS,
        SimpleEvent.COMPLETE,
    ]


def get_simple_actions(
    agent: IAgent[SimpleStage, SimpleEvent, TaskState, TaskEvent],
) -> dict[
    SimpleStage,
    Callable[
        [
            IWorkflow[SimpleStage, SimpleEvent, TaskState, TaskEvent],
            dict[str, Any],
            IQueue[Message],
            ITask[TaskState, TaskEvent],
        ],
        Awaitable[SimpleEvent]
    ]
]:
    """获取常用工作流动作定义
    ...
    Returns:
        常用工作流动作定义
    """
    actions: dict[SimpleStage, Callable[
        [
            IWorkflow[SimpleStage, SimpleEvent, TaskState, TaskEvent],
            dict[str, Any],
            IQueue[Message],
    ...
    async def reason(
        ...
        Args:
            workflow (IWorkflow[SimpleStage, SimpleEvent, TaskState, TaskEvent]): 工作流实例
        ...
        if current_state != SimpleStage.PROCESSING:
            raise RuntimeError(f"当前工作流状态错误，期望：{SimpleStage.PROCESSING}，实际：{current_state}")
        # 获取当前工作流的提示词
        ...
        return SimpleEvent.PROCESS

    # 添加到动作定义字典
    actions[SimpleStage.PROCESSING] = reason

    return actions


def build_simple_agent(name: str) -> IAgent[SimpleStage, SimpleEvent, TaskState, TaskEvent]:
    """构建一个简单的智能体实例
    ...
    Returns:
        IAgent[SimpleStage, SimpleEvent, TaskState, TaskEvent]: 智能体实例
    """
    # 获取全局设置
    ...
    llms: dict[SimpleStage, ILLM] = {}
    for stage in SimpleStage:
        # LLM 配置
    ...
    # 构建基础 Agent 实例
    agent = BaseAgent[SimpleStage, SimpleEvent, TaskState, TaskEvent](
        name=name,
    ...
    # 获取 valid states
    valid_states = get_simple_stages()
    # 获取初始状态
    init_state = SimpleStage.PROCESSING
    # 获取转换规则
    transitions = get_simple_transition()
    # 获取动作定义
    actions = get_simple_actions(agent)
    # 定义提示词
    prompts: dict[SimpleStage, str] = {
        SimpleStage.PROCESSING: read_document("prompt/workflow/simple/processing.md"),
    }
    # 定义观察函数
    ...
    observe_funcs: dict[
        SimpleStage, Callable[[ITask[TaskState, TaskEvent], dict[str, Any]], Message]
    ] = {
        SimpleStage.PROCESSING: observe_task_view,
    }
    ...
    # 构建工作流实例
    workflow = BaseWorkflow[SimpleStage, SimpleEvent, TaskState, TaskEvent](
