...
def _convert_content_to_ark_format(content: list[MultimodalContent]) -> list[ChatCompletionContentPartParam]:
        ...
        if isinstance(block, TextBlock):
            ...
            if block.image_url:
                ...
                ark_blocks.append(ChatCompletionContentPartImageParam(
                    type="image_url",
                    image_url={
                        "url": f"data:image/{block.image_type};base64,{block.image_base64}"
                    }
                ))
        elif isinstance(block, VideoBlock): # pyright: ignore[reportUnnecessaryIsInstance]
            # Video blocks might need special handling
            video_url_str = f"data:video/{block.video_type};base64,{block.video_base64}" if block.video_base64 else block.video_url or ""
...
class ArkEmbeddingLLM(IEmbedModel):
    """Ark Embedding LLM implementation (not yet supported)."""
    ...
    async def embed(
        ...
        **_kwargs: Any,
        ...
        if isinstance(content, list): # pyright: ignore[reportUnnecessaryIsInstance]
            input_params: list[EmbeddingInputParam] = []
            for item in content:
                if isinstance(item, TextBlock):
                    ...
                    image_url_str = item.image_url if item.image_url else f"data:image/{item.image_type};base64,{item.image_base64}"
                    input_params.append(
                        MultimodalEmbeddingContentPartImageParam(
                            type="image_url",
