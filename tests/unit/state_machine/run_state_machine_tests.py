...
class TestRunner:
    ...
    def run_quality_check(self) -> bool:
        ...
        pyright_success = self._run_command(f"{self.python_cmd} -m pyright tasking/state_machine")

        # 运行 pylint
        print(f"{Colors.CYAN}运行 pylint 检查...{Colors.NC}")
        pylint_success = self._run_command(f"{self.python_cmd} -m pylint tasking/state_machine")
