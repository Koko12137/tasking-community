...
This module contains comprehensive tests for the FileSystem implementation,
including line-based editing operations, file creation, file reading, and security constraints.
"""
...
from tasking.tool.filesystem import FileSystem, EditOperation
...
class TestFileSystem:
    ...
    async def terminal(self, temp_workspace):
        ...
        try:
            ...
            # Terminal initialization is already handled in constructor via syncify
            yield term
            term.close()
        finally:
    ...
    @pytest_asyncio.fixture
    async def filesystem(self, terminal):
        """Create a FileSystem instance."""
        return FileSystem(terminal_instance=terminal)
    ...
    async def test_initialization(self, terminal):
        ...
        fs = FileSystem(terminal_instance=terminal)
        assert fs._terminal == terminal
        assert fs._workspace == terminal.get_workspace()
        assert isinstance(fs._sed_inplace_arg, list)

    def test_initialization_fails_without_workspace(self):
        """Test FileSystem initialization fails without workspace."""
        mock_terminal = Mock()
        ...
        with pytest.raises(RuntimeError):
            FileSystem(terminal_instance=mock_terminal)

    @pytest.mark.asyncio
    async def test_resolve_file_path_absolute(self, filesystem, temp_workspace):
        """Test resolving absolute file paths."""
        absolute_path = os.path.join(temp_workspace, "test.txt")
        file_abs, file_rel = filesystem._resolve_file_path(absolute_path)

        assert file_abs == absolute_path
        assert file_rel == "test.txt"

    @pytest.mark.asyncio
    async def test_resolve_file_path_relative(self, filesystem, temp_workspace):
        """Test resolving relative file paths."""
        relative_path = "subdir/test.txt"
        file_abs, file_rel = filesystem._resolve_file_path(relative_path)
    ...
    @pytest.mark.asyncio
    async def test_resolve_file_path_outside_workspace(self, filesystem):
        """Test resolving paths outside workspace raises error."""
        outside_path = "/etc/passwd"

        with pytest.raises(RuntimeError):
            filesystem._resolve_file_path(outside_path)

    @pytest.mark.asyncio
    async def test_get_file_line_count_empty_file(self, filesystem, temp_workspace):
        """Test getting line count of empty file."""
        file_path = os.path.join(temp_workspace, "empty.txt")
        with open(file_path, 'w') as f:
            pass  # Create empty file

        line_count = await filesystem._get_file_line_count("empty.txt")
        assert line_count == 0

    @pytest.mark.asyncio
    async def test_get_file_line_count_nonexistent_file(self, filesystem):
        """Test getting line count of nonexistent file."""
        line_count = await filesystem._get_file_line_count("nonexistent.txt")
        assert line_count == 0

    @pytest.mark.asyncio
    async def test_get_file_line_count_multiple_lines(self, filesystem, temp_workspace):
        """Test getting line count of file with multiple lines."""
        file_path = os.path.join(temp_workspace, "lines.txt")
        with open(file_path, 'w') as f:
            f.write("line 1\nline 2\nline 3\n")

        line_count = await filesystem._get_file_line_count("lines.txt")
        assert line_count == 3

    @pytest.mark.asyncio
    async def test_ensure_parent_dir_exists(self, filesystem, temp_workspace):
        """Test ensuring parent directory exists."""
        nested_path = os.path.join(temp_workspace, "level1", "level2", "test.txt")
        await filesystem._ensure_parent_dir(nested_path)

        assert os.path.exists(os.path.join(temp_workspace, "level1", "level2"))

    def test_escape_sed_content(self, filesystem):
        """Test escaping special characters for sed."""
        # Test basic escaping
        assert filesystem._escape_sed_content("path/to/file") == "path\\/to\\/file"
        assert filesystem._escape_sed_content("replace & insert") == "replace \\& insert"
        assert filesystem._escape_sed_content(r"c:\path") == r"c:\\path"

        # Test multiline content
        multiline = "line 1\nline 2"
        escaped = filesystem._escape_sed_content(multiline)
        assert "line 1" in escaped
        assert "line 2" in escaped

    @pytest.mark.asyncio
    async def test_edit_parameter_validation(self, filesystem):
    ...
    @pytest.mark.asyncio
    async def test_edit_new_file(self, filesystem, temp_workspace):
    ...
    @pytest.mark.asyncio
    async def test_edit_modify_existing_file(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_delete_lines(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_insert_lines(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_insert_at_beginning(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_insert_at_end(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_multiple_operations(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_with_special_characters(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_multiline_content(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_subdirectory_file(self, filesystem, temp_workspace):
    ...
    @pytest.mark.asyncio
    async def test_edit_modify_nonexistent_file(self, filesystem):
    ...
    @pytest.mark.asyncio
    async def test_edit_delete_nonexistent_file(self, filesystem):
    ...
    @pytest.mark.asyncio
    async def test_edit_line_out_of_bounds(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_with_nested_directory(self, filesystem, temp_workspace):
    ...
    @pytest.mark.asyncio
    async def test_edit_preserves_file_permissions(self, filesystem, test_file_path):
    ...
    def test_terminal_security_in_filesystem_context(self, filesystem, terminal):
        """Test that terminal security constraints are enforced in FileSystem context."""
        # The FileSystem should only be able to use allowed commands
    ...
    @pytest.mark.asyncio
    async def test_filesystem_cannot_bypass_security(self, filesystem):
        """Test that FileSystem operations cannot bypass terminal security."""
    ...
    @pytest.mark.asyncio
    async def test_edit_unicode_content(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_empty_line_content(self, filesystem, test_file_path):
    ...
    @pytest.mark.asyncio
    async def test_edit_large_content(self, filesystem, test_file_path):
