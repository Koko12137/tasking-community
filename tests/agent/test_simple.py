...
测试simple.py中的功能：
- end_workflow函数
- get_simple_transition函数
- get_simple_stages函数
- get_simple_event_chain函数
- get_simple_actions函数
- build_simple_agent函数
"""
...
from src.core.agent.simple import (
    end_workflow,
    get_simple_transition,
    get_simple_stages,
    get_simple_event_chain,
    get_simple_actions,
    build_simple_agent,
    END_WORKFLOW_DOC
)
from src.core.state_machine.workflow.const import SimpleStage, SimpleEvent
...
class TestEndWorkflow(unittest.TestCase, AgentTestMixin):
    ...
    def setUp(self) -> None:
        """测试设置"""
        self.init_state, self.valid_states = TaskState.RUNNING, {TaskState.RUNNING, TaskState.FINISHED}
        self.task = self.create_mock_task(self.init_state, self.valid_states, "test-task")
...
class TestSimpleTransition(unittest.TestCase):
    """测试get_simple_transition函数"""

    def test_transition_structure(self) -> None:
        """测试转换规则结构"""
        transitions = get_simple_transition()
    ...
    def test_processing_to_process_transition(self) -> None:
        ...
        key = (SimpleStage.PROCESSING, SimpleEvent.PROCESS)
        self.assertIn(key, transitions)

        next_stage, callback = transitions[key]
        self.assertEqual(next_stage, SimpleStage.PROCESSING)
    ...
    def test_processing_to_complete_transition(self) -> None:
        ...
        key = (SimpleStage.PROCESSING, SimpleEvent.COMPLETE)
        self.assertIn(key, transitions)

        next_stage, callback = transitions[key]
        self.assertEqual(next_stage, SimpleStage.COMPLETED)
        self.assertIsNotNone(callback)


class TestSimpleStages(unittest.TestCase):
    """测试get_simple_stages函数"""

    def test_stages_content(self) -> None:
        """测试阶段集合内容"""
        stages = get_simple_stages()

        self.assertIsInstance(stages, set)
        self.assertIn(SimpleStage.PROCESSING, stages)
        self.assertIn(SimpleStage.COMPLETED, stages)
        self.assertEqual(len(stages), 2)


class TestSimpleEventChain(unittest.TestCase):
    """测试get_simple_event_chain函数"""

    def test_event_chain_structure(self) -> None:
        """测试事件链结构"""
        event_chain = get_simple_event_chain()

        self.assertIsInstance(event_chain, list)
        self.assertEqual(len(event_chain), 2)
        self.assertEqual(event_chain[0], SimpleEvent.PROCESS)
        self.assertEqual(event_chain[1], SimpleEvent.COMPLETE)


class TestSimpleActions(unittest.TestCase, AgentTestMixin):
    """测试get_simple_actions函数"""
    ...
    def setUp(self) -> None:
        ...
        # 设置get_workflow方法
        mock_workflow = Mock()
        mock_workflow.get_current_state.return_value = SimpleStage.PROCESSING
    ...
    def test_actions_structure(self) -> None:
        """测试动作结构"""
        actions = get_simple_actions(self.mock_agent)

        self.assertIsInstance(actions, dict)
        self.assertIn(SimpleStage.PROCESSING, actions)

        # 验证动作是可调用对象
        processing_action = actions[SimpleStage.PROCESSING]
    ...
    async def test_processing_action_signature(self) -> None:
        ...
        # 设置task的completion_config
        from src.model import CompletionConfig
        task.get_completion_config.return_value = CompletionConfig()
        ...
        try:
            ...
            # 验证返回值是事件类型
            self.assertIsInstance(result, (SimpleEvent, str))
...
class TestBuildSimpleAgent(unittest.TestCase):
    """测试build_simple_agent函数"""

    @patch('src.core.agent.simple.get_settings')
    @patch('src.core.agent.simple.OpenAiLLM')
    @patch('src.core.agent.simple.read_document')
    def test_build_simple_agent_basic(self, mock_read_doc, mock_openai_llm, mock_get_settings):
        """测试基本Agent构建"""
        # 设置模拟
        ...
        try:
            # 构建Agent
            agent = build_simple_agent("test-agent")
    ...
    def test_build_agent_no_config(self, mock_get_settings):
        ...
        with self.assertRaises(ValueError) as cm:
            build_simple_agent("nonexistent-agent")
