# Agent模块测试套件

这是一个完整的Agent模块测试套件，包含基础功能测试、接口测试和具体Agent实现测试，确保Agent系统的稳定性和可靠性。

## 📋 测试概览

### 测试文件结构

**test_helpers.py** - 测试辅助工具和Mock类
**test_interface.py** - Agent接口测试 (8个测试)
**test_base_agent.py** - BaseAgent核心功能测试 (15个测试)
**test_simple.py** - Simple Agent测试 (10个测试)
**test_react.py** - ReAct Agent测试 (12个测试)

**总计**: 45个测试，预计覆盖率80%+

### 1. 基础功能测试

#### 核心组件测试
- **test_interface.py**: IAgent接口定义和契约测试
- **test_base_agent.py**: BaseAgent实现和核心功能测试
- **test_helpers.py**: 测试辅助工具和Mock对象验证

#### 测试重点
- **接口契约**: 验证IAgent接口的完整性和正确性
- **BaseAgent功能**: 测试Agent基础实现的所有核心方法
- **类型安全**: 验证泛型类型约束和类型注解
- **钩子系统**: 测试完整的钩子函数机制
- **异步支持**: 验证异步方法的正确实现

### 2. Agent实现测试

#### Simple Agent测试 (test_simple.py)
- **end_workflow函数**: 工作流结束机制测试
- **状态转换**: Simple工作流状态转换规则测试
- **工具调用**: 工具调用和标签验证测试
- **Agent构建**: build_simple_agent函数测试

#### ReAct Agent测试 (test_react.py)
- **ReAct循环**: Reason-Act-Reflect循环测试
- **状态管理**: ReAct工作流状态转换测试
- **工具集成**: 工具调用和工作流集成测试
- **自定义配置**: 可配置Agent构建测试

### 3. 测试设计模式展示

#### Mock模式
- **MockLLM**: 模拟语言实现，支持成功/失败场景
- **MockTask**: 模拟任务实现，支持状态转换
- **MockWorkflow**: 模拟工作流实现
- **MockQueue**: 模拟消息队列实现

#### 异步测试模式
- **async/await支持**: 完整的异步测试支持
- **超时控制**: 测试超时保护机制
- **并发测试**: 钩子函数并发执行测试

#### 类型安全测试
- **泛型约束**: TypeVar和Generic类型测试
- **接口验证**: Protocol接口实现验证
- **类型注解**: 完整的类型注解覆盖率测试

### 4. 测试脚本设计

#### 双重脚本支持
- **run_agent_tests.sh**: Shell版本（Linux/Mac/WSL）
- **run_agent_tests.py**: Python版本（跨平台）

#### 脚本功能
- **环境智能检测**: uv和系统Python环境自动检测
- **测试分类执行**: 基础/接口/Simple/ReAct分类测试
- **代码质量检查**: 集成pyright和pylint检查
- **覆盖率报告**: HTML和XML格式覆盖率生成
- **详细日志**: 彩色输出和详细错误诊断

## 🚀 快速开始

### 环境要求
- Python 3.12+
- uv (推荐) 或 python3/pytest
- pyright (用于类型检查)
- pylint (用于代码风格检查)

### 运行测试

#### 使用Shell脚本 (推荐)
```bash
# 运行所有Agent测试
./run_agent_tests.sh all

# 运行基础功能测试
./run_agent_tests.sh basic

# 运行特定Agent测试
./run_agent_tests.sh simple
./run_agent_tests.sh react

# 生成覆盖率报告
./run_agent_tests.sh coverage

# 运行代码质量检查
./run_agent_tests.sh quality

# 查看帮助信息
./run_agent_tests.sh help
```

#### 使用Python脚本 (跨平台)
```bash
# 运行所有Agent测试
python run_agent_tests.py all

# 详细输出模式
python run_agent_tests.py all -v

# 保留覆盖率文件
python run_agent_tests.py coverage --keep

# 运行质量检查
python run_agent_tests.py quality

# 查看帮助
python run_agent_tests.py help
```

### 直接使用pytest (开发调试)
```bash
# 运行所有Agent测试
uv run pytest tests/agent/ -v

# 运行特定测试文件
uv run pytest tests/agent/test_base_agent.py -v

# 生成覆盖率报告
uv run pytest tests/agent/ --cov=src.core.agent --cov-report=term-missing
```

## 📊 测试分类详解

### 1. 基础功能测试 (basic)

#### test_interface.py
- **接口抽象性验证**: 确保IAgent是正确的抽象基类
- **方法签名检查**: 验证所有必需方法的存在和签名
- **泛型类型测试**: 测试泛型类型参数的正确使用
- **文档完整性**: 验证接口和方法的文档字符串

#### test_base_agent.py
- **基础信息管理**: ID、名称、类型管理测试
- **语言模型管理**: LLM获取和设置测试
- **工作流集成**: Workflow关联和操作测试
- **工具调用机制**: 工具调用和参数处理测试
- **任务执行流程**: 完整的任务流执行测试
- **钩子系统**: 完整的钩子函数系统测试
- **观察-思考-行动循环**: 核心Agent循环测试

### 2. Simple Agent测试 (simple)

#### 工作流管理
- **end_workflow函数**: 工作流结束和输出提取测试
- **状态转换**: Simple工作流状态转换规则测试
- **事件链**: 事件链执行顺序测试

#### Agent构建
- **build_simple_agent函数**: Agent构建和配置测试
- **依赖注入**: LLM和工作流依赖注入测试
- **错误处理**: 配置错误和异常处理测试

### 3. ReAct Agent测试 (react)

#### ReAct循环
- **Reasoning阶段**: 推理阶段功能测试
- **Reflecting阶段**: 反思阶段功能测试
- **状态转换**: ReAct工作流状态转换测试

#### 高级功能
- **自定义配置**: 可配置Agent参数测试
- **工具集成**: 工具调用和集成测试
- **错误恢复**: ReAct循环错误处理测试

## 🔧 测试工具和Mock

### MockLLM类
```python
# 创建模拟语言模型
mock_llm = MockLLM(
    model_name="test-model",
    response_content="Test response",
    should_fail=False  # 设置为True模拟错误
)
```

### MockTask类
```python
# 创建模拟任务
task = MockTask(
    initial_state="PROCESSING",
    valid_states={"PROCESSING", "COMPLETED"},
    task_id="test-task"
)
```

### AgentTestMixin混入类
```python
class MyTest(unittest.TestCase, AgentTestMixin):
    def setUp(self):
        self.mock_llm = self.create_mock_llm()
        self.task = self.create_mock_task("PROCESSING", {"PROCESSING", "COMPLETED"})
        self.workflow = self.create_mock_workflow("PROCESSING")
```

## 📈 覆盖率要求

### 覆盖率目标
- **整体覆盖率**: ≥80%
- **分支覆盖率**: ≥75%
- **接口覆盖率**: 100% (所有接口方法)

### 覆盖率报告
```bash
# 生成详细覆盖率报告
./run_agent_tests.sh coverage --keep

# 查看HTML报告
open htmlcov/index.html  # macOS
xdg-open htmlcov/index.html  # Linux
```

## 🔍 代码质量检查

### 质量检查标准
- **pyright**: 零错误，零警告
- **pylint**: 评分≥8.0/10
- **类型注解**: 100%覆盖率
- **文档字符串**: 所有公共方法

### 运行质量检查
```bash
# 运行完整质量检查
./run_agent_tests.sh quality

# 单独运行类型检查
uv run pyright src/core/agent/

# 单独运行代码风格检查
uv run pylint src/core/agent/
```

## 🚨 错误排查指南

### 常见问题

#### 1. 环境问题
**问题**: 找不到Python环境
**解决**:
```bash
# 安装uv (推荐)
pip install uv

# 或确保python3可用
python3 --version
```

#### 2. 依赖问题
**问题**: 测试依赖缺失
**解决**:
```bash
# 安装测试依赖
./run_agent_tests.sh install

# 或手动安装
uv run pip install pytest pytest-cov pytest-asyncio
```

#### 3. 导入错误
**问题**: 模块导入失败
**解决**:
- 确保在项目根目录执行测试
- 检查PYTHONPATH设置
- 使用uv run python执行

#### 4. 类型检查错误
**问题**: pyright检查失败
**解决**:
- 检查类型注解的完整性
- 确保泛型类型参数正确
- 查看具体的类型错误信息

#### 5. 异步测试超时
**问题**: 异步测试超时失败
**解决**:
- 检查Mock对象的异步方法实现
- 确保async/await语法正确
- 调整测试超时时间

### 调试技巧

#### 1. 详细输出
```bash
# 启用详细输出
./run_agent_tests.sh all -v

# Python脚本详细输出
python run_agent_tests.py all --verbose
```

#### 2. 单独运行测试
```bash
# 运行特定测试文件
./run_agent_tests.sh base_agent

# 使用pytest运行单个测试
uv run pytest tests/agent/test_base_agent.py::TestBaseAgent::test_agent_basic_info -v
```

#### 3. 保留临时文件
```bash
# 保留测试输出文件用于调试
./run_agent_tests.sh coverage --keep

# 查看pytest缓存
ls -la .pytest_cache/
```

## 📝 最佳实践

### 1. 测试编写
- 使用Mock对象隔离外部依赖
- 每个测试专注单一功能点
- 使用描述性的测试名称
- 包含正常和异常场景测试

### 2. 异步测试
- 使用`asyncio.wait_for`防止测试卡死
- 正确处理async/await语法
- 测试并发和串行执行场景

### 3. 类型安全
- 为所有Mock对象添加类型注解
- 使用TypeVar和Generic保持类型一致性
- 验证接口实现的类型正确性

### 4. 持续集成
- 所有代码编辑后必须通过质量检查
- 确保测试覆盖率达到目标要求
- 定期运行完整测试套件

## 🤝 贡献指南

### 添加新测试
1. 在相应的测试文件中添加测试方法
2. 使用AgentTestMixin提供的工具函数
3. 确保测试具有清晰的断言和错误消息
4. 更新此README文档

### 修改现有测试
1. 保持向后兼容性
2. 更新相关的Mock对象
3. 确保所有相关测试仍然通过
4. 更新文档说明

### 性能考虑
- 避免不必要的网络调用
- 使用轻量级Mock对象
- 控制测试执行时间
- 优化异步测试性能

---

**Agent测试套件遵循项目的MVP原则和奥卡姆剃刀原则，专注于核心功能验证，确保Agent系统的稳定性和可靠性。**