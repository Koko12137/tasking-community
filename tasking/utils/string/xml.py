...
def extract_by_label(content: str, *labels: str) -> str:
    ...
    按优先级顺序查找多个标签，找到第一个匹配的标签就返回其内容。
    支持多种格式：带换行符、不带换行符、自闭合标签等。

    Args:
        content (str):
            The content to extract.
        *labels (str):
            The labels to extract，按优先级顺序排列。
    ...
    for label in labels:
        for pattern in patterns:
            result = re.search(pattern.format(label=label), content, re.DOTALL | re.IGNORECASE)
            if result:
                extracted = result.group(1).strip()
                # 如果提取的内容不为空，则返回
                if extracted:
                    return extracted
        ...
        for tag_name, full_tag, position, is_closing in tags:
            ...
            if is_closing:
                ...
                if stack and stack[-1][0] == tag_name:
                    ...
                    # 多余的结束标签，需要移除
                    fixed_text = (
                        fixed_text[:adjusted_position] +
                        fixed_text[adjusted_position + len(full_tag):]
                    )
                    offset -= len(full_tag)
            else:
                # 开始标签，压入栈
